...
final class LoadAndDisplayImageTask implements Runnable
{
    ...
    private boolean delayIfNeed()
    {
        ...
        {
            log(LOG_DELAY_BEFORE_LOADING,
                    options.getDelayBeforeLoading(),
                    memoryCacheKey);
            try
            {
    ...
    private Bitmap tryLoadBitmap()
    {
        ...
        //		Log.d("LoadAndDisplayImageTask", "file path: "+imageFile.getAbsolutePath());
        Bitmap bitmap = null;
        try
        {
            if (imageFile.exists())
    ...
    private File getImageFileInDiscCache()
    {
        ...
        File imageFile = null;
        if (!TextUtils.isEmpty(options.getImageType())
                && discCache instanceof UnlimitedMultiDiscCache)
        {
            UnlimitedMultiDiscCache unlimitedMultiDiscCache = (UnlimitedMultiDiscCache) discCache;
            imageFile = unlimitedMultiDiscCache.get(uri, options.getImageType());
        }
        else
        {
    ...
    private String tryCacheImageOnDisc(File targetFile)
    {
        ...
        try
        {
            ...
            L.i(LOG_CACHE_IMAGE_ON_DISC, "uri is " + uri);
            L.i(LOG_CACHE_IMAGE_ON_DISC,
                    "targetFile path is " + targetFile.getAbsolutePath());
    ...
    private void fireImageLoadingFailedEvent(final FailType failType,
